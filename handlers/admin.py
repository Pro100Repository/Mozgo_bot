...
async def cmd_admin_help(message: Message):
    ...
    await message.answer(
        ...
        "📅 *Игры:*\n"
        "`/add_game` Название | Дата | Время | Город | Место | Ссылка на регистрацию\n\n"
        "<b>ПРИМЕР</b> (Все значения надо прописывать через <b>вертикальную черту</b>, дата и время <b>только в таком формате</b>): \n"
        "`/add_game` Классическая | 01.01.2027 | 22:00 | Мозгожопинск | В клубе | Ссылка \n\n"
        "`/list_games` — список игр с ID\n"
        "`/del_game ID` — удалить игру где <b>ID это номер игры который в списке</b>\n"
        "`/add_photo ID` - 📷 прикрепить фото к игре (Фото и команда <b>отправляется одним сообщением</b>. <b>Фото+подпись(/add_photo ID)</b>)\n\n"
        "🏆 *Результаты игр:*\n"
        "`/add_result_new` — добавить результат (пошаговая форма, <b>только для 1-3 мест</b>)\n"
