...
def skip_photo_kb(cb: str):
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="⏭ Пропустить фото", callback_data=cb)
    ]])
...
@router.message(Command("stop"), StateFilter(AddResultForm))
async def ar_stop(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("🛑 Добавление результата отменено. Ничего не сохранено в базу.")
...
async def ar_place1(message: Message, state: FSMContext):
    ...
    await state.update_data(place1_team=team, place1_photo="")
    await message.answer(
        f"🥇 1 место: *{team}*\n\nОтправь фото для этой команды или нажми «Пропустить»:",
        reply_markup=skip_photo_kb("ar_skip1"),
        parse_mode="Markdown"
    )
...
async def ar_place1_photo_received(message: Message, state: FSMContext):
...
@router.callback_query(AddResultForm.enter_place1_photo, F.data == "ar_skip1")
async def ar_place1_skip_photo(callback: CallbackQuery, state: FSMContext):
...
async def ar_place2(message: Message, state: FSMContext):
    ...
    await state.update_data(place2_team=text, place2_photo="")
    await message.answer(
        f"🥈 2 место: *{text}*\n\nОтправь фото или нажми «Пропустить»:",
        reply_markup=skip_photo_kb("ar_skip2"),
        parse_mode="Markdown"
    )
...
async def ar_place2_photo_received(message: Message, state: FSMContext):
...
@router.callback_query(AddResultForm.enter_place2_photo, F.data == "ar_skip2")
async def ar_place2_skip_photo(callback: CallbackQuery, state: FSMContext):
...
async def ar_place3(message: Message, state: FSMContext):
    ...
    await state.update_data(place3_team=text, place3_photo="")
    await message.answer(
        f"🥉 3 место: *{text}*\n\nОтправь фото или нажми «Пропустить»:",
        reply_markup=skip_photo_kb("ar_skip3"),
        parse_mode="Markdown"
    )
...
async def ar_place3_photo_received(message: Message, state: FSMContext):
    await state.update_data(place3_photo=message.photo[-1].file_id)
    await save_result(message, state)


@router.callback_query(AddResultForm.enter_place3_photo, F.data == "ar_skip3")
async def ar_place3_skip_photo(callback: CallbackQuery, state: FSMContext):
...
async def cmd_edit_result(message: Message, state: FSMContext):
    ...
    if not args or not args.isdigit():
        await message.answer("Формат: `/edit_result ID`", parse_mode="Markdown")
        return
    ...
    buttons = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏙 Город",              callback_data="er_city")],
        [InlineKeyboardButton(text="🎮 Тип игры",           callback_data="er_game_type")],
        [InlineKeyboardButton(text="📝 Название игры",      callback_data="er_game_name")],
        [InlineKeyboardButton(text="🥇 1 место (команда)",  callback_data="er_place1_team")],
        [InlineKeyboardButton(text="🥇 1 место (фото)",     callback_data="er_place1_photo")],
        [InlineKeyboardButton(text="🥈 2 место (команда)",  callback_data="er_place2_team")],
        [InlineKeyboardButton(text="🥈 2 место (фото)",     callback_data="er_place2_photo")],
        [InlineKeyboardButton(text="🥉 3 место (команда)",  callback_data="er_place3_team")],
...
@router.callback_query(EditResultForm.choose_field, F.data.startswith("er_"))
async def er_field_chosen(callback: CallbackQuery, state: FSMContext):
    field = callback.data.replace("er_", "")
    await state.update_data(edit_field=field)

    if field.endswith("_photo"):
        await callback.message.answer(
            "📷 Отправь новое фото или напиши /skip чтобы очистить:"
        )
        await state.set_state(EditResultForm.enter_photo)
    elif field == "city":
        await callback.message.answer("🏙 Выбери новый город:", reply_markup=cities_kb())
        await state.set_state(EditResultForm.enter_value)
    elif field == "game_type":
        await callback.message.answer("🎮 Выбери тип игры:", reply_markup=game_types_kb())
        await state.set_state(EditResultForm.enter_value)
    else:
...
@router.callback_query(EditResultForm.enter_value, F.data.startswith("ar_city_"))
async def er_value_city(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    value = callback.data.replace("ar_city_", "")
    await update_game_result(data["edit_id"], city=value)
    await state.clear()
    await callback.message.answer(f"✅ Город обновлён: {value}")
    await callback.answer()


@router.callback_query(EditResultForm.enter_value, F.data.startswith("ar_type_"))
async def er_value_type(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    value = callback.data.replace("ar_type_", "")
    await update_game_result(data["edit_id"], game_type=value)
    await state.clear()
    await callback.message.answer(f"✅ Тип обновлён: {RESULT_GAME_TYPES.get(value, value)}")
    await callback.answer()


@router.message(EditResultForm.enter_value)
async def er_value_text(message: Message, state: FSMContext):
    data = await state.get_data()
...
async def er_photo_received(message: Message, state: FSMContext):
    data = await state.get_data()
...
async def er_photo_clear(message: Message, state: FSMContext):
    data = await state.get_data()
